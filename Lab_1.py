...
for R in range(amount):
    ...
    for i in range(size):  # Calculating output for layer 1 nodes
        ...
        layer1[i, 0] += sigmoid(inputs[i, 0] * weight1[i, 0])

    for i in range(size):  # Calculating output for layer 2 nodes (total output in this case)

        output[0] = sigmoid(layer1[i, 0] * weight2[i])

    output[1] = output[0] * (1 - output[0]) * (target[0] - output[0])  # Error for output

    for i in range(size):
        layer1[i, 1] = layer1[i, 0] * (1 - layer1[i, 0]) * weight2[i] * output[1]  # Error for layer1 i
